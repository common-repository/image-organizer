<?php
	...
	function mbp_io_get_image_type($filename){
		$file_info=pathinfo($filename);
		return strtolower($file_info['extension']);
	}	
	
	function mbp_io_make_filename($path,$filename){
		if(substr($path,-1)=='/'){
	...
	function mbp_io_create_image($width,$height){
	...
	function mbp_io_open_image_file($path,$filename){
		$image_file_type = mbp_io_get_image_type($filename);
		
		if($image_file_type=="gif"){
			 return imagecreatefromgif(mbp_io_make_filename($path,$filename)); 
		}elseif($image_file_type=="png"){
			 return imagecreatefrompng(mbp_io_make_filename($path,$filename)); 
		}elseif($image_file_type=="jpg" || $image_file_type=="jpeg"){
			 return imagecreatefromjpeg(mbp_io_make_filename($path,$filename)); 
		}
	}

	function mbp_io_save_image_file($path,$filename,$image_res){
		$image_file_type = mbp_io_get_image_type($filename);
		if($image_file_type=="gif"){
			 imagegif($image_res,mbp_io_make_filename($path,$filename));
			 @imagedestroy($image_res);
		}elseif($image_file_type=="png"){
			 imagepng($image_res,mbp_io_make_filename($path,$filename));
			 @imagedestroy($image_res);
		}elseif($image_file_type=="jpg" || $image_file_type=="jpeg"){
			 imagejpeg($image_res,mbp_io_make_filename($path,$filename),80);
			 @imagedestroy($image_res);
		}
	}

	function mbp_io_resize_image($src_image,$resize_level){
		...
		$new_resized_image=mbp_io_create_image($resize_width,$resize_height);
		if (function_exists("imagecreatetruecolor")){
...
if($mode=='r'){
	$open_image=mbp_io_open_image_file($path,$filename);
	$white = imagecolorallocate($open_image , 255, 255, 255);
	imagefill($open_image, 0, 0, $white);
	$resized_image=mbp_io_resize_image($open_image,$level);
	...
	$image_p = mbp_io_create_image($width, $height);
